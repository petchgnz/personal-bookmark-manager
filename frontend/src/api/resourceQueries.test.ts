...
import { buildBookmarkQuery, buildPageQuery } from './resourceQueries'
...
describe('resource query builders', () => {
  ...
  it('builds a collection filter without an uncategorised filter', () => {
    expect(buildBookmarkQuery({ page: 1, limit: 20, collectionId: 'collection-id' })).toBe('page=1&limit=20&collectionId=collection-id')
  })
