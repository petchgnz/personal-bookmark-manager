...
export function useBookmarkOverview() {
  ...
  return useQuery({
    queryKey: resourceKeys.overview,
    queryFn: () => apiRequest<BookmarkOverview>('/all'),
  });
}
...
export function useCollections(page: number, limit: number, name?: string) {
  const apiRequest = useApiClient();
  return useQuery({
    queryKey: [...resourceKeys.collections, { page, limit, name }],
    queryFn: () =>
      apiRequest<PaginatedResponse<Collection>>(
        `/collections?${buildCollectionQuery(page, limit, name)}`,
      ),
  });
}
...
export function useCollection(id: string) {
  const apiRequest = useApiClient();
  return useQuery({
    queryKey: resourceKeys.collection(id),
    queryFn: () => apiRequest<Collection>(`/collections/${id}`),
  });
}
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({
        queryKey: resourceKeys.collections,
      });
  ...
  return useMutation({
    ...
    onSuccess: (collection) => {
      queryClient.setQueryData(
        resourceKeys.collection(collection.id),
        collection,
      );
      void queryClient.invalidateQueries({
        queryKey: resourceKeys.collections,
...
export function useBookmarks(filters: BookmarkFilters) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiRequest<PaginatedResponse<Bookmark>>(
        `/bookmarks?${buildBookmarkQuery(filters)}`,
      ),
  });
}
...
export function useCollectionBookmarks(
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiRequest<PaginatedResponse<Bookmark>>(
        `/collections/${id}/bookmarks?${buildPageQuery(page, limit)}`,
      ),
  });
}
...
export function useBookmark(id: string) {
  const apiRequest = useApiClient();
  return useQuery({
    queryKey: resourceKeys.bookmark(id),
    queryFn: () => apiRequest<Bookmark>(`/bookmarks/${id}`),
  });
}
...
export function useDeleteBookmark() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      apiRequest<void>(`/bookmarks/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: resourceKeys.bookmarks });
      void queryClient.invalidateQueries({
        queryKey: resourceKeys.collections,
